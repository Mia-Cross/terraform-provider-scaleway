...
func TestAccPool_ValidateRootVolume(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// Valid configuration
				Config: kapsuleClusterRepeatedConfig("validate-root-volume", latestK8SVersion) + `
			...
			},
			{
				...
				ExpectError: regexp.MustCompile("requested size must be between 20GB and 40GB, got: 100"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckK8SClusterExists(tt, "scaleway_k8s_cluster.main"),
					testAccCheckK8SPoolExists(tt, "scaleway_k8s_pool.main"),
					acctest.CheckResourceIDPersisted("scaleway_k8s_pool.main", &poolID),
		...
		Steps: []resource.TestStep{
			{
				...
				Config: kapsuleClusterRepeatedConfig("validate-size-mutu", latestK8SVersion) + `
					resource "scaleway_k8s_pool" "main" {
		...
		Steps: []resource.TestStep{
			{
				...
				Config: kapsuleDedicatedClusterRepeatedConfig("validate-size-dedi", latestK8SVersion) + `
					resource "scaleway_k8s_pool" "main" {
...
func kapsuleDedicatedClusterRepeatedConfig(testName string, version string) string {
	return fmt.Sprintf(`
		resource "scaleway_vpc" "main" {}
...
func kapsuleClusterRepeatedConfig(testName string, version string) string {
	return fmt.Sprintf(`
		resource "scaleway_vpc" "main" {}
